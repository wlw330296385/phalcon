...
class CreditReportValidation extends Validation
{
	private $type;
	...
	public function initialize(){
		...
		$this->add('name',new Validator\PresenceOf(
			['message' => '征信报告接口名称不能为空','cancelOnFail' => true]
		));
		$this->add('name', new Validator\StringLength(
			['max' => 50, 'cancelOnFail' => true, 'messageMaximum' => '征信报告接口名称内容超出限制长度']
		));
		...
		$this->add('description', new Validator\StringLength(
			['max' => 65535, 'cancelOnFail' => true,'AllowEmpty' => true, 'messageMaximum' => '征信报告接口描述内容超出限制长度']
		));
