...
class MailserverValidation extends Validation
{
	private $type;
	...
	public function initialize(){
		...
		$this->add('name',new Validator\PresenceOf(
			['message' => '邮件接口名称不能为空','cancelOnFail' => true]
		));
		$this->add('name', new Validator\StringLength(
			['max' => 50, 'cancelOnFail' => true, 'messageMaximum' => '邮件接口名称内容超出限制长度']
		));
		$this->add('type',new Validator\PresenceOf(
			['message' => '邮件发送费用类型不能为空','cancelOnFail' => true]
		));
		$this->add('type', new Validator\InclusionIn(
			['message' => '邮件发送费用类型值必须在charge,free范围内', 'cancelOnFail' => true,'domain' => ['charge','free']]
		));
