...
class ArticleValidation extends Validation
{
	...
	public function initialize(){
		...
		$this->add('title',new Validator\PresenceOf(
			['message' => '文章标题不能为空','cancelOnFail' => true]
		));
		$this->add('title', new Validator\StringLength(
			['max' => 100, 'cancelOnFail' => true, 'messageMaximum' => '文章标题内容超出限制长度']
		));
		$this->add('category_id',new Validator\PresenceOf(
			['message' => '分类编号不能为空','cancelOnFail' => true]
		));
		$this->add('category_id', new Validator\Numericality(
			['message' => '分类编号只能填写数字', 'cancelOnFail' => true,]
		));
		$this->add('category_id', new Validator\Between(
			['minimum' => 0,'maximum' => 4294967295, 'cancelOnFail' => true, 'message' => '分类编号超出限制范围']
		));
		$this->add('img', new Validator\StringLength(
			['max' => 255, 'cancelOnFail' => true,'AllowEmpty' => true, 'messageMaximum' => '文章封面内容超出限制长度']
		));
		...
		$this->add('access',new Validator\PresenceOf(
			['message' => '访问权限不能为空','cancelOnFail' => true]
		));
		$this->add('access', new Validator\InclusionIn(
			['message' => '访问权限值必须在guest,member范围内', 'cancelOnFail' => true,'domain' => ['guest','member']]
		));
		...
		$this->add('status', new Validator\InclusionIn(
			['message' => '状态值必须在regular,singular,hidden范围内', 'cancelOnFail' => true,'domain' => ['regular','singular','hidden']]
		));
