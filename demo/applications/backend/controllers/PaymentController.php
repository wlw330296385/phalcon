 ...
 * Author: Vampire
 * Email: oupula@qq.com
 * Modify:2015-12-31 22:16
 */
namespace Oupula\Backend\Controllers;
use Oupula\Backend\Library\ControllerBase;
use Oupula\Backend\Validation\PaymentValidation;
use Oupula\Models\PaymentModel;
...
{
    private $_baseNamespace = "Oupula\\Library\\Payment\\";
    ...
    public function indexAction(){
        if ($this->request->isPost()) {
            $filepath = realpath($this->config->application->commonLibraryDir . '/Payment');
            $fileArray = [];
            ...
            if (count($fileArray) > 0) {
                foreach ($fileArray as $class) {
                    ...
                    $data['module'] = $class;
                    $data['status'] = 'off';
                    if ($interface == 'FundTrustInterface') {
                        $data['type'] = 'trust';
                        $model->add($data);
                    }
                    if($interface == 'PaymentInterface'){
                        $data['type'] = 'pay';
                        $model->add($data);
                    }
                }
            ...
            $this->view->setVar('title', '支付接口接口管理');
            $this->view->enable();
        }
    }
