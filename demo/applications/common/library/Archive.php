...
class Archive {
    public $options = ['basedir' => ".",'name' => '','prepend' => "",'inmemory' => 0,'overwrite' => 0,'recurse' => 1,'storepaths' => 1,'followlinks' => 0,'level' => 3,'method' => 1,'sfx' => "",'type' => "",'comment' => ""];
    public $files = [];
    private $exclude = [];
    private $storeonly = [];
    public $error = [];
    public $archive;
    private $typelist = ['tar','gzip','bzip','zip'];
    public function __construct($name)
    {
        $this->options['name'] = $name;
        $file_info = pathinfo($name);
        if(in_array($file_info['extension'],$this->typelist)){
            $this->options['type'] = $file_info['extension'];
        }
    }
    ...
    public function create_archive()
    {
        ...
        if ($this->options['inmemory'] == 0)
        {
            ...
            {
                $this->error[] = sprintf("File %s already exists.",$this->options['name']);
                chdir($pwd);
                return 0;
            }
            ...
            {
                $this->error[] = sprintf("Could not open %s for writing.",$this->options['name']);
                chdir($pwd);
                return 0;
            }
        }
    ...
    {
        if($this->options['inmemory'] == 0){
            exit('cannot use the download_file');
        }
        switch ($this->options['type'])
        {
            case "zip":
    ...
    {
        if($this->options['type'] == "tar") {
            return @fopen($this->options['name'], "rb");
        }
        if($this->options['type'] == "gzip") {
            return @gzopen($this->options['name'], "rb");
        }
        if($this->options['type'] == 'bzip'){
        ...
        if ($fp = $this->open_archive())
        {
            ...
            while ($block = fread($fp, 512))
            {
                ...
                if ($file['checksum'] != $checksum)
                    $this->error[] = sprintf("Could not extract from %s, it is corrupt.",$this->options['name']);
            ...
            $this->error[] = sprintf("Could not open file %s",$this->options['name']);
    ...
    public function create_gzip()
    {
        if ($this->options['inmemory'] == 0)
        {
            $pwd = getcwd();
            chdir($this->options['basedir']);
            if ($fp = gzopen($this->options['name'], sprintf("wb{%s}",$this->options['level'])))
            {
    ...
    public function create_zip()
    {
        ...
        if (!empty ($this->options['sfx']))
            if ($fp = @fopen($this->options['sfx'], "rb"))
            {
                ...
                $this->error[] = sprintf("Could not open sfx module from %s.",$this->options['sfx']);
