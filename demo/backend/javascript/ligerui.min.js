...
(function ($)
{
    ...
    liger.editors = {
        ...
        "checkbox": {
            ...
            resize: function (editor, width, height, editParm)
            {
                editor.set('width', 'auto');
                $(editor.element).parent().css("width", "auto");
            }
        },
        "radio": {
            control: 'RadioList',
        ...
        "number": {
            control: 'Spinner',
            options: { format: 'float',isnegative:true,decimalplace:2,step:0.01 }
        },
        "currency": {
            control: 'Spinner',
            options: { format: 'float',isnegative:false,decimalplace:2,step:0.01 }
        },
        "integer": {
            control: 'Spinner',
            options: { format: 'int',isnegative:false }
        },
        "password": {
            control: 'TextBox',
            password: true
...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            else if ($(this.element).is(":hidden"))
            {
                g.valuefield = $(this.element);
                g.textfieldid = p.textfieldid || (this.element.id + "_txt");
                g.inputtext = $('<input type="text" readonly="true"/>');
            ...
            {
                //g.selectBox.css('width', g.wrapper.css('width')); //fix by vampire
                g.selectBox.css('width', parseInt(g.wrapper.css('width'))+9);
            }
            if (p.grid)
            {
        ...
        _setSelectBoxHeight: function (height)
        {
            ...
            else if (!p.tree)
            {
                ...
                if (p.selectboxheight)
                {
                    g.selectBoxInner.height(p.selectboxheight);//fix it by vampire
                }
            }
        },
        _setCss: function (css)
        ...
        _setWidth: function (value)
        {
            ...
            if (value > 20)
            {
                ...
                //g.inputtext.css({ width: value - 20 }); //fix by vampire
                g.inputtext.css({ width: value - 3 });
                if (!p.selectboxwidth)
                {
                    ...
                    }, onstopresize: function (current, e)
                    {
                        if (g.grid)
                        {
                            ...
                            if (current.newHeight)
                            {
                                ...
                                g._setSelectBoxHeight(current.newHeight);
                            }
                            g.grid.refreshSize();
                            g.trigger('endResize');
                            return false;
        ...
        _setValue: function (value, text)
        {
            ...
            else if (!p.ismultiselect)
            {
                ...
                $("tr[value='" + value + "'] td", g.selectBox).find('div a').addClass('l-checkbox-checked');//fix it by vampire
                $("tr[value!='" + value + "'] td", g.selectBox).removeClass("l-selected");
                $("tr[value!='" + value + "'] td", g.selectBox).find('div a').removeClass('l-checkbox-checked');//fix it by vampire
            }
            else
            {
                g._changeValue(value, text, istriggerevent);
                if (value != null)
                {
                    ...
                    $("table.l-table-checkbox :checkbox", g.selectBox).each(function () {
                        this.checked = false;
                        $(this).parent().find('a').removeClass('l-checkbox-checked');//fix it by vampire
                    });
                    for (var i = 0; i < targetdata.length; i++)
                    {
                        $("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(function () {
                            $(this).parent().find('a').addClass('l-checkbox-checked');//fix it by vampire
                            this.checked = true;
                        });
                    }
                }
            ...
            if (p.condition)
            {
                ...
                $("div", containerBtn1).ligerButton({
                    ...
                    click: function ()
                    {
                        ...
                        {
                            if (g.grid.options.url)//fix it by vampire
                            {
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p && p.absolute)
            {
                ...
                {
                    //g.selectBox.css({ left: g.wrapper.offset().left, top: g.wrapper.offset().top + 1 + g.wrapper.outerHeight() + (p.selectboxposydiff || 0) });//fix by vampire
                    g.selectBox.css({ left: g.wrapper.offset().left, top: g.wrapper.offset().top + 4 + g.wrapper.outerHeight() + (p.selectboxposydiff || 0) });
                }
                if (p.alwayshowindown)
                {
                    //g.selectBox.css({ left: g.wrapper.offset().left, top: g.wrapper.offset().top + 1 + g.wrapper.outerHeight() });//fix by vampire
                    g.selectBox.css({ left: g.wrapper.offset().left, top: g.wrapper.offset().top + 4 + g.wrapper.outerHeight() });
                }
            }
            else
            {
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _setWidth: function (value)
        {
            ...
            if (value > 20)
            {
                ...
                //g.inputtext.css({ width: value - 20 });//fix it by vampire
                g.inputtext.css({ width: value - 4 });
                g.textwrapper.css({ width: value });
            }
        },
        _setValue: function (value) //fix it by vampire
        {
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        ...
        resize: function (editor, width, height, editParm)
        {
            editor.css({
                width: width - 6
            }).parent().css("width", "auto").css('padding','2px 2px 1px 1px');
        }
    };
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        //fix it by vampire start
        var attributes = jinput.get(0).attributes;
        for(var i=0;i<attributes.length;i++) {
            ...
            else if (attributes[i]['value'] == 'false') {
                value = false;
            }
            else if (!isNaN(attributes[i]['value'])) {
                if (attributes[i]['value'].indexOf('.') != -1) {
                    value = parseFloat(attributes[i]['value']);
                } else {
                    value = parseInt(attributes[i]['value']);
                }
            }
            else if(attributes[i]['value'].indexOf('{') >= 0 && attributes[i]['value'].indexOf('}') >= 0){
                eval("value = " + attributes[i]['value'] + ";");
            }
            else{
                try{
                    eval("value = " + attributes[i]['value']);
            ...
            options[attributes[i]['name']] = value;
        }
        //fix it by vampire end
        var control = null;
        ...
        {
            var f = jinput.get(0).className.match(/ui-([a-zA-z0-9]+)/g);
            classname = (f == null) ? false : f[0];
            //var classname = jinput.get(0).className; //fix it by vampire
            if (!classname) return;
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            //分组 收缩/展开
            $("#" + $(g.element).attr('id') + " .togglebtn").live('click', function ()
            {
        ...
        _initFieldsHtml: function (e)
        {
            ...
            if (fields && fields.length)
            {
                ...
                //$(".l-group", jform).width(jform.width() * 0.95).append("<div class='togglebtn'></div>");//fix it by vampire
        ...
        _createEditors : function(e)
        {
            ...
            $(fields).each(function (fieldIndex, field)
            {
                ...
                var editorControl = g._createEditor(editor, container, {
                    field: field
                }, container.width() - 10, container.height());
        ...
        _buliderControlContainer: function (field, fieldIndex, idPrev)
        {
            ...
            {
                out.push('width:' + parseInt(width) + ';');
            }
            else if (width)
            {
                out.push('width:' + parseInt(width) + 'px;');
            }
            if (align)
            {
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        page: 1,                                //默认当前页
        pagesize: 0,                           //每页默认的结果数 pagesize == 0 时自动根据当前页面高度进行判断
        ...
        inwindow: false,                        //是否以窗口的高度为准 height设置为百分比时可用 //fix by vampire
        statusname: '__status',                    //状态名
        method: 'post',                         //获取数据http方式
        async: true,
        fixedcellheight: true,                       //是否固定单元格的高度
        heightdiff: -1,                         //高度补差,当设置height:100%时，可能会有高度的误差，可以通过这个属性调整 //fix it by vampire
        cssclass: null,                    //类名
        root: 'Rows',                       //数据源字段名
    ...
    {
        function cover_date(format, timestamp) {
            var that = this;
            var jsdate, f;
            var txt_words = [
        ...
        var format = column.format || this.options.dateformat || "Y-m-d H:i:s";
        return cover_date(format, value);
    }
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm,sourceType)
        {
            ...
            if (p.dataaction == "server")
            {
                if (p.usepager)
                {
                    var pagesize = Math.ceil(this.gridbody.height() / 30);
                    if(pagesize > this.options.pagesize && this.options.pagesize == 0){
                        this.options.pagesizeoptions.unshift(pagesize);
                        this.options.pagesize = pagesize;
                    }
                    param.push({ name: p.pageparmname, value: p.page });
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usepager)
            {
                ...
                var pagesize = Math.ceil(this.gridbody.height() / 31);
                if(pagesize > this.options.pagesize && this.options.pagesize == 0){
                    this.options.pagesizeoptions.unshift(pagesize);
                    this.options.pagesize = pagesize;
                }
                ...
                $('.l-bar-selectpagesize', g.toolbar).append("<select class='l-pagebar-select' name='rp'>" + optStr + "</select>");
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                container
                    ...
                    .show();
                if (column.textfield) editParm.text = g._getValueByName(rowdata, column.textfield);
                //var editorInput = g._createEditor(editor, container, editParm, width, height - 1);//fix it by vampire
                var editorInput = g._createEditor(editor, container, editParm, width - 10, height - 7);
                ...
                else
                {
                    ...
                    if (nextrow.length > 0)
                    {
                        ...
                        if (p.detail.onextend)
                            p.detail.onextend(item, $(".l-grid-detailpanel-inner:first", nextrow)[1]);
                    ...
                    $(src.detailbtn).addClass("l-open");
                    var frozenColNum = 0;
                    var checkboxCol = g.options.checkbox;//fix it by vampire
                    for (var i = 0; i < g.columns.length; i++){
                    ...
                    var detailFrozenRow = $("<tr class='l-grid-detailpanel'><td><div class='l-grid-detailpanel-inner' style='display:block'></div></td></tr>");
                    var checkboxRow = $("<tr class='l-grid-detailpanel'><td><div class='l-grid-detailpanel-inner' style='display:block'></div></td></tr>");
                    ...
                    g.detailrows.push(checkboxRow[0]);
                    ...
                    if(checkboxCol){
                        checkboxRow.find("td:first").attr("colSpan", 1);
                        row.eq(0).after(checkboxRow);
                    }
                    if (frozenColNum > 0)
                    {
                        detailFrozenRow.find("td:first").attr("colSpan", frozenColNum);
                        if(checkboxCol){
                            row.eq(1).after(detailRow);
                            row.eq(2).after(detailFrozenRow);
                        }else{
                    ...
                    if (p.detail.onShowDetail)
                    {
                        ...
                        if(checkboxCol){
                            $("div:first", checkboxRow).add(detailRowInner).show().height(p.detail.height || p.detailheight);
                        }
        ...
        _updateCenterBottom: function (isHeightResize)
        {
            ...
            if (g.centerBottom)
            {
                if (isHeightResize)
                {
                    ...
                    g.center.content.height(centerHeight - centerbottomheight - 22 );//fix it by vampire
                }
...
(function ($)
{
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item, menu)
        {
            ...
            item.icon && ditem.prepend('<div class="l-menu-item-icon icon ' + item.icon + '"></div>');
...
(function ($)
{
    ...
    $.ligerDefaults.Spinner = {
        ...
        format: 'int',     //fix it by vampire
        isnegative: true, //是否负数
        decimalplace: 2,   //小数位 format=float时起作用
        step: 0.1,         //每次增加的值
        interval: 50,      //间隔，毫秒
        value : null,
    ...
    $.ligerui.controls.Spinner.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            var g = this, p = this.options;
            if (p.format == 'float' || p.format == 'int') { return 0; }
            else if (p.format == 'time') { return "00:00"; }
        },
        ...
        {
            var g = this, p = this.options;
            if (p.format == 'float' || p.format == 'int')
            {
                g._addValue(p.step);
            } else if (p.format == 'time')
            {
        ...
        _isLongDateTime: function (dateStr)
        {
            ...
            var reg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2})$/;
            var r = dateStr.match(reg);
            if (r == null) return false; var d = new Date(r[1], r[3] - 1, r[4], r[5], r[6]); if (d == "NaN") return false; return (d.getFullYear() == r[1] && (d.getMonth() + 1) == r[3] && d.getDate() == r[4] && d.getHours() == r[5] && d.getMinutes() == r[6]);
        }
    });
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            {
                //g.tab.menu = $.ligerMenu({ width: 100, items: [ //fix it by vampire
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
            ...
            else if (item.type == "text")
            {
                ...
                var ditem = $('<div data-toggle="droptable" class="l-toolbar-item l-panel-btn"><span></span><div class="l-panel-btn-l"></div><div class="l-panel-btn-r"></div></div>');
                g.toolBar.append(ditem);
                if(!item.id){
                    item.id = 'itemForm_'+(++g.toolbarItemCount);
                }
                ditem.after("<ul class='dropdown-table'><form id='"+item.id+"' name='"+item.id+"' action='' method='post' class='form-horizontal' style='display:hidden'></form></ul>");
                $('div[data-toggle="droptable"]').each(function () {
                    var btn = $(this);
                    ...
                    btn.on('click', function () {
                        ...
                        table.find('.l-form-buttons').unbind('click').bind('click', function () {
                ...
                ditem.click(function(target){
                    var button = $(target.currentTarget);
                    var table = $(target.currentTarget).parent().children('.dropdown-table');
                    var offset = button.offset();
                    table.css('top',offset.top - 13);
            ...
            {
                ditem.addClass("icon l-toolbar-item-hasicon");
                ditem.addClass(item.icon);
            }
            else if (item.color)
            {
            ...
            if (item.menu)
            {
                ditem.menu = $.ligerMenu(item.menu);//fix it by vampire
                ditem.hover(function (o)
                {
                    if ($(this).hasClass("l-toolbar-item-disable")) return;
                    if(g.actionMenu){
                        g.actionMenu.hide();
                    ...
                    ditem.menu.show({ top: top, left: left });//fix it by vampire
                    g.actionMenu = ditem.menu;//fix it by vampire
                    $(this).addClass("l-panel-btn-over");
                }, function ()
                {
                    if ($(this).hasClass("l-toolbar-item-disable")) return;
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getParent: function (treenode, level)
        {
            ...
            return g._getDataNodeByTreeDataIndex(g.data,parentIndex);//fix it by vampire
        },
        //获取父节点
        ...
        _upadteTreeWidth: function ()
        {
            ...
            //treeWidth += p.nodewidth;//fix it by vampire
            //g.tree.width(treeWidth);//fix it by vampire
            if(isNaN(p.nodewidth)){
                g.tree.width(p.nodewidth);
            }else{
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (g.hasChildren(o))
                {
                    ...
                    if (p.parenticon)
                    {
                        ...
                        treehtmlarr.push('">');
                        if (p.iconfieldname && o[p.iconfieldname]){
                            //treehtmlarr.push('<img src="' + o[p.iconfieldname] + '" />');//fix it by vampire
                            treehtmlarr.push('<span class="' + o[p.iconfieldname] + '"></span>');
                        }
